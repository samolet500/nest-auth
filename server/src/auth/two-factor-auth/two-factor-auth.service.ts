/**
 * Сервис двухфакторной аутентификации: выдача одноразового кода по email и проверка.
 */
...
export class TwoFactorAuthService {
  /** Внедряет доступ к БД и отправку писем с кодом. */
  public constructor(
    private readonly prismaService: PrismaService,
    private readonly mailService: MailService,
  ) { }

  /** Проверяет код 2FA по email, удаляет токен при успехе. */
  ...
  /** Генерирует код, сохраняет токен и отправляет письмо пользователю. */
  ...
  /** Создаёт одноразовый числовой код и запись в tokens с TTL ~5 минут. */
